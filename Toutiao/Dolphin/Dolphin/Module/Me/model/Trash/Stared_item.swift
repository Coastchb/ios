...
class Stared_item : Equatable { // NSObject, NSCoding {
    ...
    static func add_stared_item(type: String, id: Int) -> Bool {
        ...
        if(!User.is_logined()) {
            ...
            var existed_ids = [Int]()
            var existed_date = [String]()
            
            if let stared_types =  UserDefaults.standard.array(forKey: STARED_TYPE_KEY) as? [String], let stared_ids = UserDefaults.standard.array(forKey: STARED_ID_KEY) as? [Int], let started_date = UserDefaults.standard.array(forKey: STARED_DATE_KEY) as? [String] {
                for i in 0..<stared_types.count {
                    var _type = stared_types[i]
                    var _id = stared_ids[i]
                    var _date = started_date[i]
                    print("\(type)--\(_type)")
                    print("\(id) -- \(_id)")
                    ...
                    existed_date.append(_date)
                }
            }
            existed_types.append(type)
            existed_ids.append(id)
            print("stared time: \(currentTime("YYYY-MM-dd HH:mm:ss"))")
            existed_date.append(currentTime("YYYY-MM-dd HH:mm:ss"))
            ...
            UserDefaults.standard.setValue(existed_date, forKey: STARED_DATE_KEY)
    ...
    static func get_all_stared_items() -> [(String,Int, String)] {
        var ret = [(String, Int, String)]()
        print("in get_all()")
